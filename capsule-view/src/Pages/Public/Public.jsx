...
const Public = () => {
 ...
 const filteredCapsules =filter === "All"? capsules : capsules.filter(capsule =>capsule.mood?.toLowerCase() === filter.toLowerCase());
 ...
 useEffect(() => {
  ...
  const fetchCapsules = async () => {
    try {
      ...
      const response = await axios.get('http://127.0.0.1:8000/api/v0.1/user/getCapsuleByPrivacy', {
        headers: {
